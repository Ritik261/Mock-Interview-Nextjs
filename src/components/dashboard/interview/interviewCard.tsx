...
function InterviewCard({ name, interviewerId, id, url, readableSlug }: Props) {
  ...
  const interviewLink = readableSlug ? `${base_url}/call/${readableSlug}` : url;
  ...
  const openCopiedLink = (e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();

    if (!interviewLink) {
      console.warn("Interview link is empty or undefined");
      return;
    }

    const newWindow = window.open(interviewLink, "_blank", "noopener,noreferrer");
